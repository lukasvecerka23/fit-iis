...
<script>
    ...
    async function fetchSystemDetail() {
            ...
            if (resp.ok){
                ...
                selectedDevices = system.devices.map(device => device.id);
            ...
            <div class="flex-col flex w-1/3 items-start">
                <div class = "flex-col flex w-full">
                    ...
                    <div class="mb-4 w-full">
                        ...
                        <input type="text" id="username" value="{system.name}" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-700" placeholder="Zadejte název systému..."/>
                    </div>
                    <div class="mb-4 w-full">
                        <label for="username" class="block mb-1 text-lg font-medium text-gray-700">Popis</label>
                        <textarea id="device-description" value="{system.description}" class="border border-gray-300 rounded-xl p-2 w-full h-40 resize-none" placeholder="Přidejte popis systému..."></textarea>
                    </div>
                ...
                <ul class="w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-slate-700 dark:border-gray-600 dark:text-white">
                    ...
                    <li class="w-full border-b border-gray-200 rounded-t-lg dark:border-slate-500">
                        <div class="flex items-center ps-3">
                            <input on:change={() => toggleDeviceSelection(device.id)} id="device-checkbox-{device.id}" type="checkbox" value="" checked={isSelected(device.id)} class="w-4 h-4 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-slate-500 dark:border-gray-500">
